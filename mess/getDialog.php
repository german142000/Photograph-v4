...
window.parent.postMessage("getDialogFrameLoad");
...
htrq.open('GET', '/1_api/api/api.php', true);
htrq.send();
...
htrq.onload = function() {
	...
	htrqr.open('GET', '/1_api/api/getDialog.php?1=<?php echo $_GET['1']?>&2=<?php echo $_GET['2']?>' +
			   '&3=' + SSresult + '&4=' + STresult + 
			   '&5=' + UIresult + '&6=' + NKresult +
	...
	htrqr.onload = function() {
		...
		if(numr[1] == '101' || numr[1] == '102') window.parent.postMessage(numr[1]);
		else {
			let data = slowAES.decrypt(JSON.parse(numr[1]), slowAES.modeOfOperation.OFB, key, iv);
			let id = slowAES.decrypt(JSON.parse(numr[2]), slowAES.modeOfOperation.OFB, key, iv);
			let date = slowAES.decrypt(JSON.parse(numr[3]), slowAES.modeOfOperation.OFB, key, iv);
			...
			for(let i = 0; i < id.length; i++)
				idString += String.fromCharCode(id[i]);
		
			let dateString = '';
			for(let i = 0; i < date.length; i++)
				dateString += String.fromCharCode(date[i]);
		
			window.parent.postMessage("dlg\n" + dataString + "\n" + idString + "\n" + dateString);
		}
	}
}
</script>
