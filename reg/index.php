...
<html>
	...
	<p class='littleLogo2' style='margin-left: 70px; '>Для продолжения вам необходимо зарегистрироваться</p>
	...
	<form action="http://photogp.unaux.com/1_api/api/reg.php" method="post">
		...
		<input style='margin-left: 80px; margin-top: 15px; cursor: pointer' type='submit' value='Зарегистрироваться'>
	</form>
	
	<p class='paragraph labelButton' style='margin-left: 80px; position: absolute; bottom: 40px;' onclick="toAuth()">
				Аутентификация</p>
	...
	<script>
		if(getCookie("trusted") == null || getCookie("trs") == null) location.href = "/";
        	...
        	htrq.open('GET', '/1_api/cid/confcid.php?1=' + trt + '&2=' + trs, true);
        	htrq.send();

        	htrq.onload = function() {
				let numr = htrq.response.split('\n', 2);
				if(numr[1] == "false") location.href = "/?path=reg";
			}
		}
		let prime = 0;
		let root = 0;
		...
		function toAuth() {
			...
			setTimeout(() => location.href = "/auth", 500);
		}
	</script>
	</body>
</html>